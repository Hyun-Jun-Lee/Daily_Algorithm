...
for i in range(n): # 첫번째 카드
    for j in range(i+1, n): # i다음 자료부터 n까지 / 2번째 카드
        for m in range(j+1, n): # j 다음 자료부터 n까지 / 3번째 카드
            res.add(card[i]+card[j]+card[m])
res = list(res) # set자료형은 sort가 불가하기 때문에 list로 바꿔줌
res.sort(reverse=True) # 내림차순 정렬
print(res[k-1]) # 0번부터 시작하니 k번째는 k-1
